...
public class AdminApiezyvectorSettingController {
    ...
    @Description("Update ezyvector settings")
    @DoPut("/settings")
    public ResponseEntity settingsPut(
        @RequestBody AdminSaveezyvectorSettingsRequest request
    ) {
        ezyvectorSettingService.setKnowledgeDataBuilderName(
            request.getKnowledgeDataBuilderName()
        );
        ezyvectorSettingService.setDataChunkerName(
            request.getDataChunkerName()
        );
        ezyvectorSettingService.setDataRetrieverName(
